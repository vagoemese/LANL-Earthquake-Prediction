...
import my_aggr_primitives as maggp
...
for cycle in tqdm_notebook(range(fault_idx.shape[0] + 1)):
    ...
    basic_prim = ['min', 'max', 'mean', 'std']
    my_prim = list(maggp.my_primitives())
    feature_matrix_cycle, feature_names = ft.dfs(entityset=es, target_entity='segment_norm',
                                           agg_primitives = basic_prim + my_prim,
...
# Save
feature_matrix.to_hdf('../generated_data/feature_matrix_1.h5', key = 'feature_matrix', mode = 'w') 
#list(feature_matrix)
